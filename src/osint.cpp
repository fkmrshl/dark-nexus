...
static double bayes_score(bool ok, bool dead_absent,
    ...
    return std::min(p, 0.99);
                          ...
                          static void lookup_getcontact(const std::string& e164, CorrelatedProfile& prof) {
                              print_section("GETCONTACT LOOKUP");
                              {
                                  ...
                                  if (resp.empty()) {
                                                  ...
                                                  if (p != std::string::npos) {
                                                      ...
                                                      if (end != std::string::npos) {
                                                          ...
                                                          if (!name.empty() && name.size() > 1) {
                                                              std::cout << GREEN << "  [+] name: " << RESET << name << "\n";
                                      ...
                                      for (auto& k : {"name","displayName","tag","firstName","lastName"}) {
                                          ...
                                          if (!v.empty() && v != "null") {
                                              std::cout << GREEN << "  [+] " << std::left << std::setw(14) << k
                                              << RESET << sanitize(v) << "\n";
                                              ...
                                              if (pos != std::string::npos) {
                                                  ...
                                                  if (end != std::string::npos && end > pos) {
                                                      ...
                                                      if (!val.empty() && val.size() > 2 && val.find("GetContact") == std::string::npos) {
                                                          std::cout << GREEN << "  [+] contact name: " << RESET << val << "\n";
                              ...
                              if (prof.names.empty()) std::cout << GRAY << "  no public data (API requires auth)\n" << RESET;
                              std::cout << YELLOW << "  tip: getcontact.com/ru/search/" << e164.substr(e164[0]=='+'?1:0) << "\n" << RESET;
                          }
                          ...
                          static void lookup_truecaller(const std::string& e164, CorrelatedProfile& prof) {
                              ...
                              std::string cc = e164.size() > 2 ? e164.substr(1,2) : "RU";
                              {
                                  ...
                                  if (InputGuard::is_safe_url(tc_url)) {
                                      ...
                                      if (!resp.empty() && resp.find("data") != std::string::npos) {
                                          ...
                                          if (!img.empty()) {
                                              std::cout << CYAN << "  [avatar]       " << RESET << sanitize(img) << "\n";
                                              prof.add("avatar", img);
                                          }
                                          std::cout << YELLOW << "  tip: truecaller.com/search/ru/" << e164.substr(e164[0]=='+'?1:0) << "\n" << RESET;
                                          return;
                                      }
                                  }
                              }
                              ...
                              if (prof.names.empty())
                                  std::cout << GRAY << "  no data (Truecaller API requires bearer token)\n" << RESET;
                              std::cout << YELLOW << "  tip: truecaller.com/search/ru/" << e164.substr(e164[0]=='+'?1:0) << "\n" << RESET;
                          }
                              ...
                              print_section("VK SEARCH BY PHONE");
                              {
                                  ...
                                  if (InputGuard::is_safe_url(vk_url)) {
                                      ...
                                      if (!resp.empty() && resp.find("\"id\"") != std::string::npos) {
                                          std::cout << GREEN << "  [+] VK profile(s) found:\n" << RESET;
                                          size_t p = 0;
                                          std::string mk = "\"id\":";
                                          int cnt = 0;
                                          while ((p = resp.find(mk, p)) != std::string::npos && cnt < 3) {
                                              ...
                                              if (!uid.empty()) {
                                                  ...
                                                  std::cout << CYAN << "    -> " << RESET << profile_url << "\n";
                                          ...
                                          for (auto& k : {"first_name","last_name","domain","city","country","bdate"}) {
                                              auto v = json_val(resp, k);
                                              if (!v.empty() && v != "null") {
                                                  std::cout << CYAN << "  [" << std::left << std::setw(12) << k << "] "
                                                  << RESET << sanitize(v) << "\n";
                                                  prof.add(k, sanitize(v));
                                              }
                                          }
                                      } else {
                                          std::cout << GRAY << "  not found via public API\n" << RESET;
                                          std::cout << YELLOW << "  tip (with token): vk.com/dev/users.search\n" << RESET;
                                      }
                                  }
                              }
                              {
                                          ...
                                          if (pos != std::string::npos) {
                                              ...
                                              if (href != std::string::npos) {
                                                  ...
                                                  if (end != std::string::npos) {
                                                      ...
                                                      if (!url.empty()) {
                                                          std::cout << GREEN << "  [+] VK web search: " << RESET << url << "\n";
                                                          prof.add("vk_web", url);
                                                      }
                                                  }
                                              }
                          ...
                          static void lookup_ok_by_phone(const std::string& e164, CorrelatedProfile& prof) {
                              ...
                              if (!resp.empty() && resp.find("\"uid\"") != std::string::npos) {
                                  for (auto& k : {"name","location","uid"}) {
                                      auto v = json_val(resp, k);
                                      if (!v.empty() && v != "null") {
                                          std::cout << GREEN << "  [+] " << std::left << std::setw(10) << k
                                          << RESET << sanitize(v) << "\n";
                                          prof.add("ok_" + std::string(k), sanitize(v));
                                      }
                                  }
                              } else {
                                  std::cout << GRAY << "  not found (OK.ru requires app token)\n" << RESET;
                              }
                          }
                              ...
                              std::string without_plus = e164.substr(e164[0]=='+'?1:0);
                              {
                                  ...
                                  if (InputGuard::is_safe_url(tg_url)) {
                                      ...
                                      if (!tg1.empty() && tg1.find("tgme_page") != std::string::npos &&
                                          ...
                                          std::cout << GREEN << "  [CONFIRMED] " << RESET << "t.me/" << e164 << "\n";
                                          ...
                                          if (pos != std::string::npos) {
                                              ...
                                              if (end != std::string::npos) {
                                                  ...
                                                  if (!name.empty() && name.find("Telegram") == std::string::npos) {
                                                      std::cout << GREEN << "  [+] display name: " << RESET << name << "\n";
                                      ...
                                      if (img != std::string::npos) {
                                          ...
                                          if (src != std::string::npos) {
                                              ...
                                              if (end != std::string::npos) {
                                                  ...
                                                  if (!photo_url.empty()) {
                                                      std::cout << CYAN << "  [+] profile photo: " << RESET << photo_url << "\n";
                                              ...
                                              std::cout << GRAY << "  [-] no Telegram profile at t.me/" << e164 << "\n" << RESET;
                                          }
                                  }
                              }
                              {
                                  ...
                                  if (InputGuard::is_safe_url(tg2_url)) {
                                      ...
                                      if (!tg2.empty() && tg2.find("tgme_page") != std::string::npos &&
                                          ...
                                          std::cout << GREEN << "  [CONFIRMED] " << RESET << "t.me/+" << without_plus << "\n";
                                      prof.add("telegram_plus", "t.me/+" + without_plus);
                                          }
                                  }
                              }
                              std::cout << GRAY << "  [-] WhatsApp / Signal / Viber: no public lookup API\n" << RESET;
                          }
...

                              {
                                  ...
                                  if (InputGuard::is_safe_url(ddg_url)) {
                                      ...
                                      if (!ddg.empty()) {
                                          ...
                                          if (!shown) std::cout << GRAY << "  no public web mentions\n" << RESET;
                                      }
                                  }
                              }
                                                               }
                                                                   ...
                                                                   print_section("LINKED PHONE EXTRACTION");
                                                                   {
                                                                       ...
                                                                       if (InputGuard::is_safe_url(grav_url)) {
                                                                           ...
                                                                           if (!grav.empty() && grav.find("phoneNumbers") != std::string::npos) {
                                                                               ...
                                                                               if (pos != std::string::npos) {
                                                                                   ...
                                                                                   if (val_pos != std::string::npos) {
                                                                                       ...
                                                                                       if (end != std::string::npos) {
                                                                                           ...
                                                                                           if (!phone.empty()) {
                                                                                               std::cout << GREEN << "  [+] Gravatar phone: " << RESET << phone << "\n";
                                                                               ...
                                                                               std::cout << GRAY << "  Gravatar: no phone in public profile\n" << RESET;
                                                                           }
                                                                       }
                                                                   }
                                                                   {
                                                                       ...
                                                                       if (InputGuard::is_safe_url(vk_url)) {
                                                                           ...
                                                                           if (!vk.empty() && vk.find("\"id\"") != std::string::npos) {
                                                                               std::cout << GREEN << "  [+] VK: email found in profile\n" << RESET;
                                                                               ...
                                                                               std::cout << GRAY << "  VK: email not found in public profiles\n" << RESET;
                                                                           }
                                                                       }
                                                                   }
                                                                   {
                                                                       ...
                                                                       if (InputGuard::is_safe_url(gh_url)) {
                                                                           ...
                                                                           if (!hub.empty() && hub.find("\"login\"") != std::string::npos) {
                                                                               std::cout << GREEN << "  [+] GitHub: profile linked to this email\n" << RESET;
                                                                               ...
                                                                               while ((p = hub.find("\"login\":\"", p)) != std::string::npos && cnt < 3) {
                                                                                   ...
                                                                                   if (end != std::string::npos) {
                                                                                       ...
                                                                                       std::cout << CYAN << "  [github] " << RESET << "github.com/" << login << "\n";
                                                                           ...
                                                                           if (!li.empty() && li.find("profile-name") != std::string::npos) {
                                                                               ...
                                                                               if (pos != std::string::npos) {
                                                                                   ...
                                                                                   if (end != std::string::npos) {
                                                                                       ...
                                                                                       if (!chunk.empty())
                                                                                           std::cout << GREEN << "  [+] LinkedIn match found\n" << RESET;
                                                                                   }
                                                                               }
                                                                           }
                                                                       }
                                                                                                            ...
                                                                                                            static void correlate_and_print(CorrelatedProfile& prof,
                                                                                                                ...
                                                                                                                std::cout << CYAN << "  [seed]       " << RESET << seed << "\n\n";

                                                                                                                if (!prof.names.empty()) {
                                                                                                                    std::cout << BOLD << WHITE << "  NAMES FOUND:\n" << RESET;
                                                                                                                    for (auto& n : prof.names)
                                                                                                                        std::cout << GREEN << "  [+] " << RESET << n << "\n";
                                                                                                                    std::cout << "\n";
                                                                                                                }

                                                                                                                if (!prof.phones.empty()) {
                                                                                                                    std::cout << BOLD << WHITE << "  PHONE NUMBERS:\n" << RESET;
                                                                                                                    for (auto& p : prof.phones)
                                                                                                                        std::cout << GREEN << "  [+] " << RESET << p << "\n";
                                                                                                                    std::cout << "\n";
                                                                                                                }

                                                                                                                if (!prof.emails.empty()) {
                                                                                                                    std::cout << BOLD << WHITE << "  EMAIL ADDRESSES:\n" << RESET;
                                                                                                                    for (auto& e : prof.emails)
                                                                                                                        std::cout << GREEN << "  [+] " << RESET << e << "\n";
                                                                                                                    std::cout << "\n";
                                                                                                                }

                                                                                                                if (!prof.locations.empty()) {
                                                                                                                    std::cout << BOLD << WHITE << "  LOCATIONS:\n" << RESET;
                                                                                                                ...
                                                                                                                if (!prof.accounts.empty()) {
                                                                                                                    std::cout << BOLD << WHITE << "  LINKED ACCOUNTS:\n" << RESET;
                                                                                                                    for (auto& a : prof.accounts)
                                                                                                                        std::cout << GREEN << "  [+] " << RESET << a << "\n";
                                                                                                                    std::cout << "\n";
                                                                                                                }
                                                                                                                ...
                                                                                                                std::cout << CYAN << "  [completeness] " << RESET
                                                                                                                << std::fixed << std::setprecision(0) << (completeness*100) << "%  "
                                                                                                                << conf_bar(completeness) << "\n";
                                                                                                                std::cout << CYAN << "  [confirmed]    " << RESET << confirmed << " accounts\n";
                                                                                                                std::cout << CYAN << "  [probable]     " << RESET << probable  << " accounts\n";
                                                                                                                std::cout << CYAN << "  [data_points]  " << RESET << prof.raw.size() << "\n";

                                                                                                                if (!prof.names.empty() && !prof.locations.empty())
                                                                                                                    std::cout << GREEN << "\n  [IDENTITY] " << RESET
                                                                                                                    << prof.names[0] << " -- " << prof.locations[0] << "\n";
                                                                                                                else if (!prof.names.empty())
                                                                                                                    std::cout << GREEN << "\n  [IDENTITY] " << RESET << prof.names[0] << "\n";
                                                                                                                                            }
                                                                                                                                            ...
                                                                                                                                            static void run_phone(const std::string& phone_raw, IdentityGraph& graph) {
                                                                                                                                                    ...
                                                                                                                                                    if (cc->code == "+7") {
                                                                                                                                                        ...
                                                                                                                                                        if (ln.size() == 11) {
                                                                                                                                                            ...
                                                                                                                                                            if (it != RU_OPERATORS.end()) {
                                                                                                                                                                std::cout << GREEN << "  [operator]   " << RESET << it->second.op
                                                                                                                                                                << GRAY << " (prefix " << prefix << ")\n" << RESET;
                                                                                                                                                                std::cout << GREEN << "  [reg_region] " << RESET << it->second.region << "\n";
                                                                                                                                                                graph.profile.add("operator", it->second.op);
                                                                                                                                                                graph.profile.add("registration_region", it->second.region);
                                                                                                                                                            } else {
                                                                                                                                                                std::cout << YELLOW << "  [operator]   unknown prefix " << prefix << "\n" << RESET;
                                                                                                                                                            }

                                                                                                                                                            auto ci = RU_CITY_CODES.find(prefix);
                                                                                                                                                            if (ci != RU_CITY_CODES.end()) {
                                                                                                                                                                std::cout << CYAN << "  [city_hint]  " << RESET << ci->second << "\n";
                                                                                                                                                            ...
                                                                                                                                                            for (int len : {4, 3}) {
                                                                                                                                                                ...
                                                                                                                                                                if (lci != RU_CITY_CODES.end()) {
                                                                                                                                                                    std::cout << CYAN << "  [city_exact] " << RESET << lci->second << "\n";
                                                                                                                                                        ...
                                                                                                                                                        if (!nl.empty() && nl != "{}" && nl.find("error") == std::string::npos) {
                                                                                                                                                            ...
                                                                                                                                                            if (InputGuard::is_safe_url(tw_url)) {
                                                                                                                                                                ...
                                                                                                                                                                if (!tw.empty() && tw.find("carrier") != std::string::npos) {
                                                                                                                                                                    ...
                                                                                                                                                                    if (!carrier.empty()) {
                                                                                                                                                                        std::cout << CYAN << "  [carrier]    " << RESET << sanitize(carrier) << "\n";
                                                                                                                                                                        graph.profile.add("carrier", sanitize(carrier));
                                                                                                                                                                    }
                                                                                                                                                                    if (!type.empty()) {
                                                                                                                                                                        std::cout << CYAN << "  [line_type]  " << RESET << sanitize(type) << "\n";
                                                                                                                                                                        graph.profile.add("line_type", sanitize(type));
                                                                                                                                                                    }
                                                                                                                                                                } else {
                                                                                                                                                                    std::cout << GRAY << "  carrier API unavailable (no key)\n" << RESET;
                                                                                                                                                                }
                                                                                                                                                            }
                                                                                                                                                        }
                                                                                                                                                    }
                                                                                                                                                ...
                                                                                                                                                if (flags.empty()) std::cout << GREEN << "  [ok] no suspicious patterns\n" << RESET;
