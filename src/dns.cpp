...
void dns_lookup(const std::string& domain) {
    ...
    for(auto& ip:a_ips){
        ...
        std::cout<<RED<<"  [A]    "<<WHITE<<std::left<<std::setw(18)<<sanitize(ip);
        if(!ptr.empty()) std::cout<<RED<<"  PTR -> "<<WHITE<<sanitize(ptr);
        std::cout<<RESET<<"\n";
    }
    for(auto& ip:aaaa_ips) std::cout<<RED<<"  [AAAA] "<<WHITE<<sanitize(ip)<<RESET<<"\n";
    if(a_ips.empty()&&aaaa_ips.empty()) std::cout<<RED<<"  no A/AAAA records\n"<<RESET;
    ...
    for(auto& f:futs){
        auto [t,out]=f.get();
        std::cout<<"\n"<<RED<<"  ["<<WHITE<<t<<RED<<"]:\n"<<RESET;
        if(out.empty()) std::cout<<RED<<"  none\n"<<RESET;
    ...
    for(auto& line:split_lines(txt)){
        if(line.find("v=spf1")!=std::string::npos){
            std::cout<<RED<<"  [SPF] "<<WHITE<<sanitize(line)<<RESET<<"\n";
            spf_found=true;
            ...
            while(std::regex_search(s,m,re_inc)){
                ...
                std::cout<<RED<<"    -> "<<WHITE<<sanitize(inc)<<RESET<<"\n";
    ...
    if(dmarc.empty()) std::cout<<RED<<"  [-] no DMARC\n"<<RESET;
    else std::cout<<WHITE<<sanitize(dmarc)<<RESET;

    std::cout<<RED<<"\n  -- DNSSEC --\n"<<RESET;
    auto ds=dig_short(domain,"DS",5);
    auto dnskey=dig_short(domain,"DNSKEY",5);
    if(!ds.empty()||!dnskey.empty()) std::cout<<RED<<"  [+] "<<WHITE<<"DNSSEC enabled\n"<<RESET;
    else std::cout<<RED<<"  [-] DNSSEC not detected\n"<<RESET;
    ...
    for(auto ns:split_lines(ns_raw)){
        ...
        std::cout<<RED<<"  trying "<<WHITE<<sanitize(ns)<<RESET<<"\n";
        dns_rl.acquire();
        ...
        if(zt.empty()||zt.find("Transfer failed")!=std::string::npos||zt.find("REFUSED")!=std::string::npos)
            ...
            std::cout<<RED<<"  [!!!] ZONE TRANSFER SUCCEEDED on "<<WHITE<<sanitize(ns)<<RED<<"!\n"<<WHITE<<sanitize(zt)<<RESET;
    }
    ...
    while(std::getline(ss,line)){
        ...
        for(auto& k:keys){
            ...
            if(c!=std::string::npos)
                std::cout<<RED<<"  ["<<std::left<<std::setw(16)<<line.substr(0,c)<<"] "<<WHITE<<sanitize(line.substr(c+1))<<RESET<<"\n";
            break;
        }
    }
}
...
void site_lookup(const std::string& raw) {
    ...
    for(auto& p:{"https://","http://","www."}) {
        ...
        std::cout<<RED<<"  resolving "<<WHITE<<s<<RED<<"...\n"<<RESET;
        std::string ip=resolve(s);
        if(ip.empty()){std::cout<<RED<<"  could not resolve\n"<<RESET;return;}
        std::cout<<RED<<"  "<<WHITE<<s<<RED<<" -> "<<WHITE<<ip<<"\n"<<RESET;
        ip_intel(ip);
}
